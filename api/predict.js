// api/predict.js - DEPRESSEDESIGN Trading Station v6.1
...
async function sendZoneAlertEnhanced(zone, extraMsg = "") {
  ...
  const typeIconMap = { FVG:"🔷", OB:"🔶", PHP:"⬜", PHL:"⬜", LIQ:"💧", BRK:"🔀", SESSION:"🕐", CLUSTER:"🔗" };
  const typeIcon = typeIconMap[zone.type] || "📍";
...
function isDisplacementChoCH(o, h, l, c, direction) {
  ...
  return direction === "buy" ? curr.c > prev.h && curr.c > curr.o : curr.c < prev.l && curr.c < curr.o;
}
...
function updatePendingZones(zones, currentTime) {
  ...
  for (const zone of zones) {
    if (zone.strength >= 4 && !pendingZones.has(zone.id)) {
      pendingZones.set(zone.id, { zone: { ...zone }, createdAt: currentTime, hitCount: 0 });
    }
  }
}
...
function scanPreviousHL(h1, session) {
  ...
  swings.swingHighs.slice(-5).forEach((sh, i) => {
    ...
    const strength = 5 - i;
    zones.push({
      type: "PHP", typeLabel: "Previous High (Supply)", bias: "SELL",
      high: zHigh, low: zLow, strength: Math.min(5, Math.max(1, strength)),
  ...
  swings.swingLows.slice(-5).forEach((sl, i) => {
    ...
    const strength = 5 - i;
    zones.push({
      type: "PHL", typeLabel: "Previous Low (Demand)", bias: "BUY",
      high: zHigh, low: zLow, strength: Math.min(5, Math.max(1, strength)),
  ...
  for (let i = len - 30; i < len - 2; i++) {
    ...
    if (bullGapLow > bullGapHigh) {
      ...
      if (!filled && Math.abs(price - midpoint) < atr * 3) {
        ...
        zones.push({
          type: "FVG", typeLabel: "Fair Value Gap (Demand)", bias: isBelowPrice ? "BUY" : "TARGET",
    ...
    if (bearGapHigh < bearGapLow) {
      ...
      if (!filled && Math.abs(price - midpoint) < atr * 3) {
        ...
        zones.push({
          type: "FVG", typeLabel: "Fair Value Gap (Supply)", bias: isAbovePrice ? "SELL" : "TARGET",
...
function scanBreakers(m5, session) {
  ...
  for (let i = len - 50; i < len - 5; i++) {
    ...
    if (isBullCandleHere) {
      ...
      zones.push({
        type: "BRK", typeLabel: "Breaker Block (Flipped Support)", bias: midpoint < price ? "BUY" : "TARGET",
    ...
    if (isBearCandleHere) {
      ...
      if (!retestedBreaker) continue;
      zones.push({
        type: "BRK", typeLabel: "Breaker Block (Flipped Resistance)", bias: midpoint > price ? "SELL" : "TARGET",
    ...
    for (let j = i+2; j < len - 1; j++) {
      ...
      if (diffH < tolerance) {
        ...
        if (dist < atr * 10 && dist > atr * 0.5) {
          zones.push({
            type: "LIQ", typeLabel: "Liquidity Level (Equal Highs)", bias: abovePrice ? "SELL" : "TARGET",
      ...
      if (diffL < tolerance) {
        ...
        if (dist < atr * 10 && dist > atr * 0.5) {
          zones.push({
            type: "LIQ", typeLabel: "Liquidity Level (Equal Lows)", bias: "BUY",
...
function scanSessionLevels(h1, session) {
  ...
  if (utcH >= 7 && utcH <= 20) {
    ...
    if (dist < atr * 6) {
      zones.push({
        type: "SESSION", typeLabel: "Session Level (London Open)", bias: price > londonOpen ? "SELL" : "BUY",
...
async function checkEntryTriggers(m1, zones, session) {
  ...
  for (const zone of zones) {
    ...
    triggers.push({
      bias: zone.bias, zoneType: zone.typeLabel, zoneBias: zone.bias,
      entry: entryPrice, sl, tp1, tp2,
      tp1Pips: Math.abs(tp1 - entryPrice).toFixed(0),
      tp2Pips: Math.abs(tp2 - entryPrice).toFixed(0),
      confluence, session, zoneId: zone.id,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      upcoming_news: events.filter(e => (e.country === "US" || e.currency === "USD") && new Date(e.date).getTime() > Date.now() - 3600000)
