...
// Vercel Serverless Function (Node.js) - V21 (ULTIMATE GLOBAL & ALERT FIX)
...
// ─── MATHEMATICAL & TECHNICAL INDICATORS ENGINE (V21) ────────────────────────
...
function calculateRSI(closes, period = 14) { let gains = 0, losses = 0; for(let i=1; i<=period; i++) { let diff = closes[i] - closes[i-1]; if(diff>=0) gains += diff; else losses -= diff; } let avgGain = gains/period; let avgLoss = losses/period; for(let i=period+1; i<closes.length; i++) { let diff = closes[i] - closes[i-1]; if(diff>=0){ avgGain = (avgGain*13 + diff)/14; avgLoss = (avgLoss*13)/14; } else { avgGain = (avgGain*13)/14; avgLoss = (avgLoss*13 - diff)/14; } } let rs = avgGain/avgLoss; return 100 - (100/(1+rs)); }
...
// ─── THE NEW INSTITUTIONAL GOD ENGINE ────────────────────────────────────────
    ...
    if (h1Trend === "BULLISH" && currentPrice <= (buyTarget + 2.0) && currentPrice >= (buyTarget - 1.5)) {
      if (m1Rsi < 40) { 
        ...
        reasonArr = [
          ...
          `VOLATILITY: Stop Loss & Target dilindungi oleh ATR Dinamis (${m1Atr.toFixed(2)} pts)`
        ];
      }
    }
      ...
      reasonArr = [
        ...
        `LTF (M1): RSI saat ini di ${m1Rsi.toFixed(1)}. Menunggu momentum.`
      ];
    }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ─── PERBAIKAN TELEGRAM ALERT (BERJALAN DI CRON MAUPUN WEB REFRESH) ───
    ...
    if (isCron) {
      // PRE-NEWS WARNING TETAP DI DALAM CRON AGAR TIDAK SPAM KETIKA WEB DIRE-FRESH
      const upcomingNews = events.filter(e => {
        // Warning telegram fokus ke impact tinggi aja (Boleh ganti if you want)
        const diffMins = (new Date(e.date).getTime() - nowMs) / 60000;
      ...
      for (const news of upcomingNews) {
        ...
        if (!warnedEvents.has(eventId)) {
          // Filter hanya yang high impact untuk Telegram Pre-news
